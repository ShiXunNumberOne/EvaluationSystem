...
<script type="text/javascript">
    ...
    var addChildNormtarget = function(data){
        var addIndex = layer.open({
            title : "添加评教指标",
            type : 2,
            content : "${base}/admin/system/normtarget/add?pid="+data,
            success : function(layero, addIndex){
                setTimeout(function(){
    ...
    var delNormtarget =function(data){
        layer.confirm("你确定要删除该指标类别么？这将会使得其下的所有指标都删除",{btn:['是的,我确定','我再想想']},
            function(){
                $.post("${base}/admin/system/normtarget/delete",{"id":data},function (res){
                    if(res.success){
